// ─────────────────────────────────────────────
//  CIVIQ — Navbar
//  Inside white content card — sits on white/dark surface
//  Title: large bold — maximum contrast both modes
//  Icons: generous spacing gap-3
//  Dark toggle · Bell · Avatar+name
// ─────────────────────────────────────────────
...
export default function Navbar({
  ...
  return (
    <div className="flex items-center justify-between px-8 h-14 border-b border-[#E2E8F0] dark:border-[#1E293B] flex-shrink-0">

      {/* LEFT — page title, large bold, max contrast */}
      <h1 className="text-[18px] font-bold text-[#0F172A] dark:text-[#F8FAFC] tracking-tight leading-none">
        {pageTitle}
      </h1>

      {/* RIGHT — generous gap between all icons */}
          ...
          {showProfile && (
            <>
              ...
              <div className="absolute right-0 top-11 w-52 rounded-[8px] py-1 z-50 bg-[#FFFFFF] dark:bg-[#1C1C1F] border border-[#E2E8F0] dark:border-[#1E293B] shadow-[0_8px_24px_rgba(0,0,0,0.08)] dark:shadow-[0_8px_24px_rgba(0,0,0,0.40)]">
                ...
                <button className="w-full text-left px-4 py-2.5 text-[13px] text-[#DC2626] hover:bg-[#FEF2F2] dark:hover:bg-[#2D0A0A] transition-colors">
                  Sign out
                </button>
              </div>
            </>
